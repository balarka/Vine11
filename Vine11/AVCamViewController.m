...
- (void)viewDidLoad
{
	...
	if ([self captureManager] == nil) {
		...
		if ([[self captureManager] setupSession]) {
            ...
            CGRect ct = CGRectMake(0, 410, 70, 50);
            self.recordButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
            [self recordButton].frame = ct;
            ...
            ct = CGRectMake(230, 410, 90, 50);
            self.previewButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
            [self previewButton].frame = ct;
            [[self previewButton] setTitle:@"Preview >" forState:UIControlStateNormal];
            [[self previewButton] addTarget:self action:@selector(previewVideo:) forControlEvents:UIControlEventTouchDown];
            [view addSubview: [self previewButton]];
            
            
//            [[self cameraToggleButton] setTitle:@"Camera" forState:UIControlStateNormal];
//            [[self stillButton] setTitle:@"Photo" forState:UIControlStateNormal];
...
{
    NSLog(@"File is stored at %@", [[[self captureManager] recorder] outputFileURL]);
    PreviewVideoViewController *pv = [[PreviewVideoViewController alloc] init];
    [pv setVideoFileURL: [[[self captureManager] recorder] outputFileURL]];
